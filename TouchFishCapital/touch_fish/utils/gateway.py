...
from touch_fish.event import Event, EventEngine
from .event import (
    EVENT_TICK,
    EVENT_ORDER,
    EVENT_TRADE,
...
class BaseGateway(ABC):
    ...
    * on_contract
    ...
    def __init__(self, event_engine: EventEngine, gateway_name: str):
        ...
        self.orders: Dict[str, OrderData] = {}  # Order List
        self.trades: Dict[str, TradeData] = {}
        self.traded_orders: Dict[str, List] = defaultdict(list)
    ...
    def on_trade(self, trade: TradeData) -> None:
        ...
        """
        """
            Process the returned trade info
        """
        # add a new trade into self.trades
        ...
        # compute the slippage and trading cost of the trade
        traded_info = self.traded_orders[trade.vt_orderid]
        traded_info.append(trade)
        self.traded_orders[trade.vt_orderid] = traded_info

        trade.order_price = self.orders[trade.vt_orderid].price
        # Calculate trading cost and slippage for each trade record
        direction = 1 if trade.direction == Direction.LONG else -1
        trade.slippage = int(((trade.price - trade.order_price) * direction)/pricetick)*pricetick
        # trade_pricetick = int(trade.volume * pricetick/pricetick) * pricetick
        # trade.trading_cost = int((trade.volume * trade.slippage)/trade_pricetick)*trade_pricetick
        trade.trading_cost = trade.volume * trade.slippage

        self.on_event(EVENT_TRADE, trade)
        self.on_event(EVENT_TRADE + trade.vt_symbol, trade)

        # compute the slippage and trading cost of the order
        order = self.orders[trade.vt_orderid]

        if order.traded > 0:
            # Calculate average trading price

            traded_vols = [single_trade.volume for single_trade in self.traded_orders[order.vt_orderid]]
            traded_prices = [single_trade.price for single_trade in self.traded_orders[order.vt_orderid]]
            order.avg_trade_price = sum([a*b for a, b in zip(traded_vols, traded_prices)]) / order.traded
            # Calculate trading costs
            direction = 1 if order.direction == Direction.LONG else -1
            order.slippage = int(((order.avg_trade_price - order.price) * direction)/pricetick)*pricetick

            # order_pricetick = int(pricetick*order.traded/pricetick)*pricetick
            # order.trading_cost = int((order.traded * order.slippage)/order_pricetick)*order_pricetick
            order.trading_cost = order.traded * order.slippage
    ...
    def on_order(self, order: OrderData) -> None:
        ...
        if order.vt_orderid not in self.orders:
            ...
            delta_traded = order.traded - self.orders[order.vt_orderid].traded
            if delta_traded != 0:
                self.orders[order.vt_orderid] = order
            else:
                self.orders[order.vt_orderid] = order
